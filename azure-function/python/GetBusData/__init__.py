...
def main(GetBusData: func.TimerRequest) -> None:
    ...
    logging.info(f"AZURE_CONN_STRING: {AZURE_CONN_STRING}")
    ...
    monitored_buses = [
        get_monitored_format(bus["vehicle"])
        for bus in entities
        if get_route_id(bus) in monitored_routes
    ]
    logging.info(
        f"{len(entities)} buses found. {len(monitored_buses)} buses monitored."
    )
    ...
    for fence in geo_fences:
        logging.info(
            f"Vehicle {fence['VehicleId']}, route {fence['RouteId']}, status: {fence['GeoFenceStatus']} at {fence['TimestampUTC']} UTC"
        )
